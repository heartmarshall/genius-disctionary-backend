...
Reads output/{show}/dataset.csv for each show in config.yaml.

Output:
    output/{show}/unique.csv  — words exclusive to one show (word, total_count)
    output/common_words.csv   — words in ALL shows (word, total, per-show counts)
...
def load_show_words(show_name: str) -> dict[str, int]:
    ...
    csv_path = show_dir(show_name) / "dataset.csv"
...
def write_unique(show_name: str, unique: dict[str, int]) -> None:
    ...
    output_path = show_dir(show_name) / "unique.csv"
...
def main() -> None:
    ...
    if len(shows) < 2:
        print("Error: need at least 2 shows in config.yaml", file=sys.stderr)
        sys.exit(1)
