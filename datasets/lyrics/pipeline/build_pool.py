#!/usr/bin/env python3
"""Build deduplicated common word pool from all artist datasets.

Usage:
    python build_pool.py

For each artist: reads words.txt, subtracts exclude.csv.
Merges remaining words into a deduplicated pool.

Output:
    output/common_pool.csv — word, artists, artist_count
"""
...
def main() -> None:
    ...
    word_artists: dict[str, list[str]] = defaultdict(list)

    for name, label in artists.items():
        dir_path = artist_dir(name)
        ...
        print(
            f"{label:>20}: {len(all_words):>6} total"
            f" - {len(exclude):>5} exclude"
            f" = {len(clean):>6} clean"
        )

        for w in clean:
            word_artists[w].append(label)

    rows = sorted(word_artists.items(), key=lambda x: (-len(x[1]), x[0]))

    out_path = ROOT / "output" / "common_pool.csv"
    with open(out_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["word", "artists", "artist_count"])
        for word, artist_list in rows:
            writer.writerow(
                [word, "|".join(sorted(artist_list)), len(artist_list)]
            )
    ...
    for _, artist_list in rows:
        by_count[len(artist_list)] += 1
    print("\nDistribution:")
    for n in sorted(by_count):
        print(f"  in {n} artist(s): {by_count[n]} words")
