...
import (
	...
	cardservice "github.com/heartmarshall/my-english/internal/service/card"
	...
	topicservice "github.com/heartmarshall/my-english/internal/service/topic"
...
func (r *dictionaryEntryResolver) Topics(ctx context.Context, obj *model.DictionaryEntry) ([]*model.Topic, error) {
	loaders, err := dataloader.MustFor(ctx)
	if err != nil {
		return nil, transport.HandleError(ctx, err)
	}
	items, err := loaders.TopicsByEntryID.Load(ctx, obj.ID)
	if err != nil {
		return nil, transport.HandleError(ctx, err)
	}
	res := make([]*model.Topic, len(items))
	for i := range items {
		res[i] = &items[i]
	}
	return res, nil
...
func (r *mutationResolver) CreateTopic(ctx context.Context, input model1.CreateTopicInput) (*model.Topic, error) {
	topic, err := r.Services.Topic.CreateTopic(ctx, mapCreateTopicInput(input))
	if err != nil {
		return nil, transport.HandleError(ctx, err)
	}
	return topic, nil
...
func (r *mutationResolver) UpdateTopic(ctx context.Context, input model1.UpdateTopicInput) (*model.Topic, error) {
	topic, err := r.Services.Topic.UpdateTopic(ctx, mapUpdateTopicInput(input))
	if err != nil {
		return nil, transport.HandleError(ctx, err)
	}
	return topic, nil
...
func (r *mutationResolver) DeleteTopic(ctx context.Context, id uuid.UUID) (bool, error) {
	err := r.Services.Topic.DeleteTopic(ctx, topicservice.DeleteTopicInput{ID: id.String()})
	if err != nil {
		return false, transport.HandleError(ctx, err)
	}
	return true, nil
...
func (r *mutationResolver) AddTopics(ctx context.Context, entryID uuid.UUID, topicIDs []uuid.UUID) (*model.DictionaryEntry, error) {
	topicIDStrings := make([]string, len(topicIDs))
	for i, id := range topicIDs {
		topicIDStrings[i] = id.String()
	}
	err := r.Services.Dictionary.AddTopics(ctx, dictservice.AddTopicsInput{
		EntryID:  entryID.String(),
		TopicIDs: topicIDStrings,
	})
	if err != nil {
		return nil, transport.HandleError(ctx, err)
	}
	// Возвращаем обновленную запись
	entry, err := r.Services.Dictionary.GetByID(ctx, entryID)
	if err != nil {
		return nil, transport.HandleError(ctx, err)
	}
	return entry, nil
...
// RemoveTopics is the resolver for the removeTopics field.
func (r *mutationResolver) RemoveTopics(ctx context.Context, entryID uuid.UUID, topicIDs []uuid.UUID) (*model.DictionaryEntry, error) {
	topicIDStrings := make([]string, len(topicIDs))
	for i, id := range topicIDs {
		topicIDStrings[i] = id.String()
	}
	err := r.Services.Dictionary.RemoveTopics(ctx, dictservice.RemoveTopicsInput{
		EntryID:  entryID.String(),
		TopicIDs: topicIDStrings,
	})
	if err != nil {
...
func (r *mutationResolver) CreateCard(ctx context.Context, entryID uuid.UUID) (*model.Card, error) {
	createdCard, err := r.Services.Card.CreateCard(ctx, cardservice.CreateCardInput{
		EntryID: entryID.String(),
	})
	if err != nil {
		return nil, transport.HandleError(ctx, err)
...
// ImportDictionary is the resolver for the importDictionary field.
func (r *mutationResolver) ImportDictionary(ctx context.Context, file graphql.Upload) (*model1.ImportReport, error) {
	// Читаем файл из Upload структуры
	reader := file.File

	// Вызываем сервис импорта
	report, err := r.Services.Impex.Import(ctx, reader)
	if err != nil {
		return nil, transport.HandleError(ctx, err)
	}

	// Маппим отчет из impex в GraphQL модель
	return &model1.ImportReport{
...
func (r *queryResolver) Topics(ctx context.Context) ([]*model.Topic, error) {
	topics, err := r.Services.Topic.ListTopics(ctx)
	if err != nil {
		return nil, transport.HandleError(ctx, err)
	}
	res := make([]*model.Topic, len(topics))
	for i := range topics {
		res[i] = &topics[i]
	}
	return res, nil
...
func (r *queryResolver) Topic(ctx context.Context, id uuid.UUID) (*model.Topic, error) {
	topic, err := r.Services.Topic.GetByID(ctx, id)
	if err != nil {
		return nil, transport.HandleError(ctx, err)
	}
	return topic, nil
...
// ExportDictionary is the resolver for the exportDictionary field.
func (r *queryResolver) ExportDictionary(ctx context.Context) (string, error) {
	jsonBytes, err := r.Services.Impex.Export(ctx)
	if err != nil {
		return "", transport.HandleError(ctx, err)
	}
